...
var Config = (function () {
    function gulpConfig() {
		...
		this.allConfig = this.source + '/*.json';
		this.tokenpub = this.source + '/*.pub';
...
gulp.task('compile-ts-dest', ['ts-lint', 'compile-ts'], function () {
    return gulp.src([config.source + '/js/**/*.*',config.allConfig,config.tokenpub])
...
gulp.task('minify', ['ts-lint', 'compile-ts'], function () {
    gulp.src(config.allJavaScript)
	  ...
	  return gulp.src([config.allConfig,config.tokenpub]).pipe(gulp.dest(config.dist));
});
